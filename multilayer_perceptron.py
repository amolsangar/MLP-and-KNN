...
# Reference - https://pabloinsente.github.io/the-multilayer-perceptron
...
class MultilayerPerceptron:
    ...
    def _initialize(self, X, y):
        ...
        self._h_weights = np.random.uniform(-1, 1, size=(n_features,self.n_hidden))
        self._h_bias = np.random.uniform(1, 1, size=(1,self.n_hidden))

        self._o_weights = np.random.uniform(-1, 1, size=(self.n_hidden,n_outputs))
        self._o_bias = np.random.uniform(1, 1, size=(1,n_outputs))
    ...
    def fit(self, X, y):
        ...
        for _ in range(self.n_iterations):
            # Forward-propagation
            Z1 = self.linear_function(self._h_weights, X, self._h_bias)
            S1 = self.hidden_activation(Z1)
            Z2 = self.linear_function(self._o_weights, S1, self._o_bias)
            S2 = self._output_activation(Z2)

            # Error computation
            if(_ % 20 == 0):
                error = self._loss_function(self._y, S2)
                self._loss_history.append(error)
            
            # Backpropagation
            delta2 = (S2 - self._y) * self._output_activation(S2,True)
            W2_gradients = S1.T @ delta2

            # Standardize gradients to avoid explosion and then clip values
            W2_gradients = (W2_gradients - np.mean(W2_gradients)) / np.std(W2_gradients)
            np.clip(W2_gradients, -1, 1, out=W2_gradients)
            
            # update output weights
            self._o_weights = self._o_weights - (W2_gradients * self.learning_rate)

            # ===========================================================================

            delta1 = (delta2 @ self._o_weights.T ) * self.hidden_activation(S1,True)
            W1_gradients = X.T @ delta1
            
            # Standardize gradients to avoid explosion and then clip values
            W1_gradients = (W1_gradients - np.mean(W1_gradients)) / np.std(W1_gradients)
            np.clip(W1_gradients, -1, 1, out=W1_gradients)

            # update hidden weights
            self._h_weights = self._h_weights - (W1_gradients * self.learning_rate)
